...
const RenderHeadingTree = ({ tree, indent }: { tree: HeadingTree[]; indent: number }) => {
    ...
    // Sub-headings collapse into a single running line, magazine-contents style
    if (!isTopLevel) {
        return (
            <ul className="mt-1.5 flex list-none flex-wrap items-baseline gap-x-3 gap-y-1 pl-10">
                {tree.map((e) => (
                    <li key={e.id} className="text-foreground/75 text-sm leading-relaxed">
                        <NextLink
                            href={`#${e.id}`}
                            scroll={true}
                            className="hover:text-accent inline-block py-1 transition-colors duration-150"
            ...
            {tree.map((e, index) => (
                <li key={e.id} className="leading-relaxed">
                    <NextLink
                        href={`#${e.id}`}
                        scroll={true}
                        className="group hover:text-accent flex items-baseline gap-4 transition-colors duration-150"
                    >
                        <span
                            className="text-accent w-6 shrink-0 text-right text-[10px] font-semibold tracking-[0.1em] tabular-nums"
                            aria-hidden
                        >
                            {(index + 1).toString().padStart(2, "0")}
                        </span>
                        <span className="font-display text-base font-medium" dangerouslySetInnerHTML={{ __html: e.text }} />
                    </NextLink>
